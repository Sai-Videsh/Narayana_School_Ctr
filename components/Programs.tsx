...
export default function Programs() {
  return (
    <section id="programs" className="py-32 bg-brand-blue relative overflow-hidden">
      {/* Background patterns */}
      ...
      <div className="container mx-auto px-6 relative z-10">
        <div className="mb-20 text-center">
          <motion.h2 
            ...
            className="font-display font-medium text-4xl md:text-7xl tracking-tight text-white uppercase mb-8"
          >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          {programs.map((p, i) => (
            <motion.div
              ...
              whileHover={{ y: -10 }}
              className="p-10 bg-white rounded-3xl border border-black/10 hover:border-black/30 transition-all duration-700 shadow-2xl group"
            >
              <div className={`w-20 h-20 rounded-2xl ${p.accent} flex items-center justify-center mb-8 group-hover:scale-110 transition-transform duration-500 shadow-inner`}>
                {p.icon}
              </div>
              <h3 className="text-2xl font-display font-bold text-dark mb-4 group-hover:text-brand-orange transition-colors uppercase">{p.title}</h3>
              <p className="text-gray-600 leading-relaxed text-sm mb-8">{p.desc}</p>
              <a 
