...
export default function Results() {
  return (
    <section id="results" className="py-24 bg-surface text-dark overflow-hidden relative">
      {/* Decorative Glow */}
      ...
      <div className="container mx-auto px-6 relative z-10">
        <div className="flex flex-col lg:flex-row gap-16 items-center">
          <div className="lg:w-1/2">
            <h2 className="font-display font-medium text-5xl md:text-7xl leading-tight mb-8">
              RESULTS THAT <br />
            ...
            <div className="grid grid-cols-2 gap-8">
              {stats.map((stat, i) => (
                <div key={i}>
                  <p className="text-4xl md:text-5xl font-medium mb-1 text-brand-orange">{stat.value}</p>
          ...
          <div className="lg:w-1/2 grid grid-cols-2 gap-6">
            {toppers.map((topper, i) => (
              <motion.div
                ...
                viewport={{ once: true }}
                className="bg-white p-0 rounded-3xl aspect-[4/5] flex flex-col justify-end border-2 border-transparent group hover:border-brand-orange/50 transition-all duration-700 shadow-xl overflow-hidden relative"
              >
                <div 
                ...
                <div className="relative z-20 p-6 text-white translate-y-2 group-hover:translate-y-0 transition-transform duration-500">
                  <p className="text-[10px] font-bold text-brand-orange uppercase mb-1 tracking-widest">{topper.exam}</p>
                  <p className="font-bold text-lg leading-tight mb-1">{topper.name}</p>
                  <p className="text-2xl font-display font-bold text-brand-blue">{topper.rank}</p>
                </div>
              </motion.div>
            ))}
          </div>
